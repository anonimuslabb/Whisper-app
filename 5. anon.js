...
function loadLinkFromURL() \{\
    ...
    \
    if (!token) \{\
        showError('Neplatn\'fd odkaz - chyb\'ed token.');\
        return;\
    \}\
    \
    ...
    \
    if (!linkData) \{\
        showError('Odkaz nebyl nalezen nebo nen\'ed aktivn\'ed.');\
        return;\
    \}\
    \
    if (!linkData.isActive) \{\
        showError('Tento odkaz ji\'9e byl pou\'9eit a nen\'ed aktivn\'ed.');\
        return;\
    \}\
    \
    displayLinkInfo();\
...
function displayLinkInfo() \{\
    ...
    if (linkInfo && linkData) \{\
        ...
        if (linkData.effects && linkData.effects.length > 0) \{\
            effectsText = ` \'95 Dostupn\'e9 efekty: $\{linkData.effects.join(', ')\}`;\
        \}\
        \
        linkInfo.innerHTML = `\
            <div style="background: rgba(5, 150, 105, 0.1); padding: 12px; border-radius: 8px; margin: 16px 0;">\
                \uc0\u55357 \u56525  <strong>$\{linkData.name\}</strong>$\{effectsText\}\
            </div>\
        `;\
    \}\
\}\
...
function toggleEffect(effect, buttonElement) \{\
    if (linkData.effects && !linkData.effects.includes(effect)) \{\
        showError(`Efekt $\{effect\} nen\'ed dostupn\'fd pro tento odkaz.`);\
        return;\
    \}\
    \
    ...
    \} else \{\
        if (selectedEffects.length >= 3) \{\
            showError('M\uc0\u367 \'9eete vybrat maxim\'e1ln\u283  3 efekty.');\
            return;\
        \}\
...
function updateSelectedEffectsDisplay() \{\
    ...
    \
    selectedEffectsEl.innerHTML = `<div style="margin-top: 8px;">Vybran\'e9 efekty: $\{effectsHtml\}</div>`;\
\}\
\
function handleSendMessage(e) \{\
    ...
    if (!messageText) \{\
        playSound('error');\
        showError('Napi\'9ate n\uc0\u283 jakou zpr\'e1vu!');\
        return;\
    \}\
    \
    if (!linkData || !linkData.isActive) \{\
        playSound('error');\
        showError('Odkaz nen\'ed aktivn\'ed!');\
        return;\
    \}\
    \
    const message = \{\
